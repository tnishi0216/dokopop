...
HWND hwndOrg = NULL;
ATSOCR atsocr;
bool RequireHwndATSOCR = false;
bool OnlyATSOCR = false;
bool tryATSOCR = false;
bool MoveSend = false;
#if USE_DBW
int attach = 0;
#endif
	...
	if (!tryATSOCR){
		...
		if (atsocr.GetHandle()){
			...
			if (GetWindowThreadProcessId(hwnd, &procId)){
				...
				if (hProc){
					memset(ATSOCRPath, 0, sizeof(ATSOCRPath));
					if (GetModuleFileNameEx(hProc, NULL, ATSOCRPath, sizeof(ATSOCRPath))>0){
						// OK
					}
					CloseHandle(hProc);
				}
...
int WINAPI Config2( const struct TDCHConfig *cfg )
{
	...
	OnlyATSOCR = int_bool(cfg->OnlyATSOCR);
...
{
	DBW("DoCapture: %d %d %d %d", image_only, runOnLaunchedProc, atsocr.GetHandle(), OnlyATSOCR);
	CursorPoint = ScreenPoint = pt;
	ScreenToClient( hwnd, &CursorPoint );

#if 1	// acrobat reader上でclickするとhwndATSOCRがnullになってしまう場合があるため（原因不明）
	...
	if (!image_only && !runOnLaunchedProc){
		...
		if (!atsocr.GetHandle() || !OnlyATSOCR){
			// Redraw Metafile //
			UNPROTECT_SHARE();
	...
	if (image_only || atsocr.GetHandle()){
...
bool CaptureImage(HWND hwnd, bool movesend, bool non_block)
{
	...
	if (atsocr.GetHandle()){
		capture_page = SendMessage(atsocr.GetHandle(), WM_ATSOCR, ATSOCR_CMD_PAGE_CAPTURE, 0) ? true : false;
	}
...
void CheckATSOCRAlive()
{
	CallbackMain(DCH_LAUNCH_ATSOCR, NULL, 0, 0);
}
